...
def revision(repo, ref):
    return subprocess.check_output(["git", "-C", str(repo), "rev-parse", "--verify", "--end-of-options", ref + "^{commit}"], text=True).strip()
...
def stage_fixtures(folder, inputs):
    if not folder.is_dir() or folder.is_symlink():
        raise ValueError("fixture directory is missing or is a symlink: %s" % folder)
    sources = sorted(folder.rglob("*.xml"))
    if not sources:
        raise ValueError("fixture directory contains no XML builds: %s" % folder)
    for source in sources:
        if any((folder / part).is_symlink() for part in source.relative_to(folder).parents):
            raise ValueError("invalid fixed fixture")
        if source.is_symlink() or not source.resolve().is_relative_to(folder.resolve()) or source.stat().st_size > 4 * 1024 * 1024:
            raise ValueError("invalid fixed fixture")
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="pob-corpus-") as scratch:
        ...
        def calculate(job):
            ...
            print("Calculating %s %s" % (destination.name, batch), flush=True)
        ...
        compared = subprocess.run(command, timeout=300)
        if compared.returncode == 1 and args.strict:
            print("Calculated all %d input pairs; differences detected (strict mode)" % len(names), flush=True)
            return 1
        compared.check_returncode()
    print("Completed calculation and comparison of %d input pairs in %.1fs" % (len(names), time.monotonic()-started), flush=True)
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("Build calculation/comparison error (%s): %s" % (type(exc).__name__, exc), flush=True)
        raise SystemExit(2)
