...
class RunnerTests(unittest.TestCase):
    def test_nested_fixtures_are_included_and_duplicate_bytes_deduplicated(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            fixtures, inputs = root / 'fixtures', root / 'inputs'
            (fixtures / '3.13').mkdir(parents=True)
            inputs.mkdir()
            xml = b'<PathOfBuilding><Build/></PathOfBuilding>'
            (fixtures / '3.13' / 'one.xml').write_bytes(xml)
            (fixtures / 'copy.xml').write_bytes(xml)
            runner.stage_fixtures(fixtures, inputs)
            self.assertEqual([p.read_bytes() for p in inputs.iterdir()], [xml])
            with self.assertRaises(ValueError):
                runner.stage_fixtures(root / 'typo', inputs)
            empty = root / 'empty'
            empty.mkdir()
            with self.assertRaises(ValueError):
                runner.stage_fixtures(empty, inputs)
